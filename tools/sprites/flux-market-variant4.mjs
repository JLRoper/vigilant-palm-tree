// FLUX isometric covered market hall - variant 4.
//
// Usage:
//   $env:DEEPINFRA_API_KEY = "..." ; node tools/sprites/flux-market-variant4.mjs
...
const JOBS = [
  {
    name: "building-classic-market-1-variant4.png",
    seed: 10120,
    prompt: "Single isolated pixel art isometric fantasy covered market hall with dark wooden beam structure and orange clay tile roof, open sides with hanging fabric, wooden tables with bread loaves and pottery displayed, warm lantern light inside, cobblestone floor, viewed from 3/4 angle, dark saturated colors, fills the entire canvas, pixel art, black outlines",
  },
];
...
for (const job of JOBS) {
  ...
  try {
    ...
    const raw = await gen(job.prompt, job.seed);
