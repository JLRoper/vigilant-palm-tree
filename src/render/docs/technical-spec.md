...
- **Still to build**: `scene/paint2d/` (the actual painter reading `SceneNode[]`), and the `renderer.ts`/`cityRenderer.ts` rewrite to actually consume `SceneNode[]` (deliberately last — the only step touching the live render path).
...
- **Known pitfall for `paint2d/`** (confirmed *not* a factor for `battleScene.ts`, whose only imports are `core/hex.ts`, `@heroes/engine`, and the sibling `scene/types.ts`): importing from `cityRenderer.ts`, or from the `cityBuildingDraw.ts` barrel (which pulls in `assetDescriptors.ts`'s dozens of Vite `?url` PNG imports), crashes under plain `node:test`/tsx — Node has no loader for `.png`/`?url` specifiers outside Vite's bundler. Import pure helpers from their actual leaf module instead (e.g. `cityBuildingDraw/primitives.ts` for `buildingFootprint`, `core/cityGrid.ts` for `computeCityScale`), not through an asset-loading barrel.
