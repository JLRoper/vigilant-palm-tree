...
// Procedurally-modelled unit sprites, generated by `npm run models:build` from
// tools/models/units/*.ts. Only the `still/` directory is globbed -- the
// `anim/` strips are not wired into a painter yet, and globbing them would
// bundle them into the build for nothing. See docs/unit-models.md.
const UNIT_STILL_GLOB = import.meta.glob(
  "../resources/units/*/still/*.png",
  { eager: true }
) as Record<string, { default: string }>;

const UNIT_SPRITE_NATURAL_SIZE = 128;

export const UNIT_DESCRIPTORS: Record<string, SpriteDescriptor> = {};

for (const [filePath, mod] of Object.entries(UNIT_STILL_GLOB)) {
  const match = filePath.match(/\/units\/([^/]+)\/still\/[^/]+-(ne|nw|se|sw|n|e|s|w)\.png$/);
  if (!match || !mod.default) continue;
  const unitTypeId = match[1];
  const direction = match[2] as Direction;
  const key = unitKey(unitTypeId, direction);
  UNIT_DESCRIPTORS[key] = {
    key,
    url: mod.default,
    anchor: "bottom",
    // Capped so a unit is no taller than the hex row pitch (1.5 * hexSize):
    // the rendered figure fills ~73% of its (square, bottom-aligned) frame, so
    // 2.0 keeps vertically adjacent platoons from systematically overlapping.
    sizing: { kind: "fitHeight", hexSizeMul: 2.0 },
    naturalSize: UNIT_SPRITE_NATURAL_SIZE,
  };
}

export function unitKey(unitTypeId: string, direction: Direction): `unit.${string}.${Direction}` {
  return `unit.${unitTypeId}.${direction}` as `unit.${string}.${Direction}`;
}

export function hasUnitSprite(unitTypeId: string, direction: Direction): boolean {
  return unitKey(unitTypeId, direction) in UNIT_DESCRIPTORS;
}
...
export const ALL_DESCRIPTORS: readonly SpriteDescriptor[] = [
  ...
  ...Object.values(UNIT_DESCRIPTORS),
