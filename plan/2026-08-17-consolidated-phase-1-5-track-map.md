...
| Decompose `src/screens/combat/manualBattleArena.ts` into modular components (4-step plan in `plan/2026-08-17-combat-decomposition-finishing-breakout.md`) | 🟡 **CB-1 + CB-2 merged (PR #112 + PR #113)** — `arena/{index,constants,layout,view,input,leaveBehind,state,ai}.ts` extracted (orchestrator unchanged, public API identical, `-180 lines` from `manualBattleArena.ts`, now 1384 lines per PR #113); `test/screens/combat/arena.test.ts` (12 tests, Proxy-based DOM mock) added. ⬜ **CB-3/4 still pending** — CB-3 ready (`manualBattleArena.ts` still has the inline `draw()`+`renderRails()`+input wiring; orchestrator does not yet delegate to `arena/openManualBattleArena.ts`); CB-4 (the post-paint2d `arena/paint.ts` extraction behind the `useSceneBuilder` flag). |
...
4. **Phase 5.B continuation** — `adventureScene.ts`, `cityScene.ts`, `entityMirror.ts`, `battleScene.ts`, and `paint2d/` (dispatcher shell + dep seam) are done and unit-tested (§7.2); the `manualBattleArena.ts` decomposition has CB-1 + CB-2 merged (PR #112 + PR #113). Remaining, roughly in ascending risk order: `paint2d/` per-kind Canvas transcription (the 28 stubs → real Canvas calls, Commits 3-10 of the design doc) plus `src/render/paint2dDefaults.ts` and `src/render/skybox.ts` (Commit 2, which is the only file allowed to touch the Vite-coupled assets); then CB-3/4 of `manualBattleArena.ts` decomposition (CB-3 = shrink `manualBattleArena.ts` to a thin orchestrator that delegates to `arena/openManualBattleArena.ts`; CB-4 = `arena/paint.ts` extraction behind the `useSceneBuilder` flag, blocked on paint2d/ per-kind transcription); and finally rewriring `renderer.ts`/`cityRenderer.ts` to actually consume `SceneNode[]` (deliberately last — the only step touching the live render path).
