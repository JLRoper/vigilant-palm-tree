...
Phase 5  Client Event Sync & Scene Renderer Seam        [🟡 IN PROGRESS]
   ...
   └── 5.B  Scene graph builder & entity-mirror animation [🟡 Dev B — adventureScene/cityScene/entityMirror/battleScene done+tested; paint2d dispatcher shell + seam done; paint2d per-kind transcription, manualBattleArena decomp (CB-1 scaffolding in PR #112, CB-2/3/4 still ⬜), renderer rewrite still ⬜]
```
...
| Decompose `src/screens/combat/manualBattleArena.ts` into modular components (4-step plan in `plan/2026-08-17-combat-decomposition-finishing-breakout.md`, not yet present in this worktree) | 🟡 **CB-1 scaffolding in PR #112 (open, not merged)** — `src/screens/combat/arena/{index,constants,layout,view,input,leaveBehind}.ts` extracted (orchestrator unchanged, public API identical, `-622 lines` from `manualBattleArena.ts`); `test/screens/combat/arena.test.ts` (12 tests, Proxy-based DOM mock) added; `package.json` `test:unit` glob widened to `test/screens/**/*.test.ts`. ⬜ **CB-2/3/4 still pending** — CB-2 (unblocked by CB-1, ready to start), CB-3 (fill the `arena/index.ts` barrel), CB-4 (caller-side cutover). |
...
4. **Phase 5.B continuation** — `adventureScene.ts`, `cityScene.ts`, `entityMirror.ts`, `battleScene.ts`, and now `paint2d/` (dispatcher shell + dep seam) are done and unit-tested (§7.2). Remaining, roughly in ascending risk order: `paint2d/` per-kind Canvas transcription (the 28 stubs → real Canvas calls, Commits 3-10 of the design doc) plus `src/render/paint2dDefaults.ts` and `src/render/skybox.ts` (Commit 2, which is the only file allowed to touch the Vite-coupled assets); then CB-2/3/4 of `manualBattleArena.ts` decomposition (CB-1 scaffolding in PR #112 open — pure pure-helper + DOM-only + leave-behind extraction done, target structure now exists; CB-2/CB-3/CB-4 still pending alongside the new plan doc's CB-1); and finally rewriring `renderer.ts`/`cityRenderer.ts` to actually consume `SceneNode[]` (deliberately last — the only step touching the live render path).
