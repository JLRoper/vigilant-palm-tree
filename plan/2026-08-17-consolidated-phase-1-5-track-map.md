...
Phase 5  Client Event Sync & Scene Renderer Seam        [🟡 IN PROGRESS]
   ...
   └── 5.B  Scene graph builder & entity-mirror animation [🟡 Dev B — adventureScene/cityScene/entityMirror/battleScene done+tested; paint2d dispatcher shell + seam done; paint2d per-kind transcription, manualBattleArena decomp, renderer rewrite still ⬜]
```
...
3. ~~**P0 repo-health batch**~~ — done, open as 🟡 **PR #111** (§11): `docker/Dockerfile` workspace-COPY fix (image builds again — verified by actually running `docker build` on both runtime targets, which had been failing since the `shared/` → `packages/engine` rename), `engines` pin + guarded Playwright `postinstall`, README prerequisites, and this doc's bookkeeping/symbol corrections. No app-code changes.

**Remaining, in recommended order:**

4. **Decide event-cursor/SSE ownership** ⬜ — *the one true blocker, and the recommended next step.* `server/routes.ts:475`'s `GET /games/:name/events` still does `SELECT ... ORDER BY id ASC` with no `?after=` filter (verified), and §8's ownership matrix assigns that server-side slice to **neither track**. Everything else in Track 5.A queues behind it: `multiplayerSync.ts`'s rewrite (still a full-state poller — `api.getGame()` → `hydrateGameState()`, verified at `src/io/multiplayerSync.ts:60-65`), `GameSessionManager.ts`'s cursor init, and retiring `SessionManager.manualSave()`'s full-state push. This is cheap to decide and expensive to keep deferring — it has been unowned since Phase 5.A started. Note the retirement of `manualSave()` also needs a `test/smoke.ts` answer, since that suite asserts on the Save button + "Last saved" HUD text.
5. **`paint2d/` per-kind Canvas transcription** ⬜ — the 33 remaining stub/TODO markers in `src/render/scene/paint2d/index.ts` (verified count) → real Canvas calls, Commits 3–10 of the design doc, plus Commit 2's `src/render/paint2dDefaults.ts` and `src/render/skybox.ts` (neither file exists yet — verified; they are the *only* files allowed to touch the Vite-`?url` assets). Highest-volume remaining work but lowest risk: still additive/unwired, so it cannot regress the live render path.
6. **`manualBattleArena.ts` decomposition** ⬜ — deliberately after (5): vague scope, no pre-agreed target structure, and it is a live interactive screen.
7. **`renderer.ts`/`cityRenderer.ts` rewrite to consume `SceneNode[]`** ⬜ — deliberately last; the only step that touches the live render path. Both files still have zero `SceneNode`/`paintScene` references (verified). PR #110's live in-browser render capture (adventure map + battle arena, zero console errors, all assets 200 OK) is the baseline to diff against when this lands.

**Tracked but not blocking (P2):** the #89 follow-up — unit-level regression assertions in `commandHandler.test.ts` that the `settlement_snapshots`/`resource_transactions` writes actually fire (§5.1, still ⬜; the SQL strings present in that file today are fake-pool-client dispatch handlers, not assertions) — worth promoting from doc footnote to a real tracked issue. Also: branch cleanup (57 remote branches, at least one name reused across two unrelated PRs).
