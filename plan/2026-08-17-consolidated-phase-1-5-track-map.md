...
Phase 5  Client Event Sync & Scene Renderer Seam        [🟡 IN PROGRESS]
   ...
   └── 5.B  Scene graph builder & entity-mirror animation [🟡 Dev B — adventureScene/cityScene/entityMirror/battleScene done+tested; paint2d dispatcher shell + seam done; manualBattleArena decomp (CB-1 + CB-2 + CB-3 merged in PR #112 + #113 + #115 — orchestrator is now a thin shim and lives in arena/openManualBattleArena.ts; CB-4 still 🚫 deferred behind paint2d/ per-kind transcription), paint2d per-kind transcription, renderer rewrite still ⬜]
```
...
| Decompose `src/screens/combat/manualBattleArena.ts` into modular components (4-step plan in `plan/2026-08-17-combat-decomposition-finishing-breakout.md`) | 🟡 **CB-1 + CB-2 + CB-3 merged (PR #112 + #113 + #115)** — `arena/{index,constants,layout,view,input,leaveBehind,state,ai,openManualBattleArena}.ts` extracted (orchestrator now delegates from `arena/openManualBattleArena.ts`; `manualBattleArena.ts` shrunk from 1514 lines to a 16-line shim, `-1498` net); `test/screens/combat/arena.test.ts` (12 tests, Proxy-based DOM mock) added. 🚫 **CB-4 still deferred** (post-paint2d `arena/paint.ts` extraction behind `useSceneBuilder` flag, blocked on `paint2d/` per-kind transcription, 5.B P1 #5). |
...
| R6 | `BuildStructure` blocked on missing engine validate+apply | ∞ | Stage 6 prerequisite. Track whenever next engine module is added. Not the same gap as `UpgradeBuilding` (existing-building level-up) — that reducer already existed and just needed a command; ✅ closed 2026-08-18 via PR #118, see §5.1. |
...
3. ~~**`UpgradeSettlement`/`UpgradeBuilding` command ports + issue #114**~~ — done, pending merge (PR #118, §5.1, §10 R6, revision note 9). Merge #118 to actually close issues #88 and #114 on GitHub.
4. **Phase 5.A continuation** — `src/io/commands.ts` exists and R4 is closed (§7.1). Remaining: decide who owns adding `?after=<seq>` cursor support to the events endpoint (or an SSE alternative) — not currently assigned in the ownership matrix (§8) — then rewrite `multiplayerSync.ts` against it, wire `GameSessionManager.ts`'s cursor init, and either replace or deliberately retire `SessionManager.manualSave()`'s full-state push (currently still exercised by `test/smoke.ts`'s Save-button assertion).
5. **Phase 5.B continuation** — `adventureScene.ts`, `cityScene.ts`, `entityMirror.ts`, `battleScene.ts`, and `paint2d/` (dispatcher shell + dep seam) are done and unit-tested (§7.2); the `manualBattleArena.ts` decomposition has CB-1 + CB-2 + CB-3 merged (PRs #112, #113, #115). Remaining, roughly in ascending risk order: `paint2d/` per-kind Canvas transcription (the 28 stubs → real Canvas calls, Commits 3-10 of the design doc) plus `src/render/paint2dDefaults.ts` and `src/render/skybox.ts` (Commit 2, which is the only file allowed to touch the Vite-coupled assets); then CB-4 (`arena/paint.ts` extraction behind the `useSceneBuilder` flag, 🚫 deferred, blocked on paint2d/ per-kind transcription); and finally rewriring `renderer.ts`/`cityRenderer.ts` to actually consume `SceneNode[]` (deliberately last — the only step touching the live render path).
6. **New follow-up surfaced by PR #118's review:** no client-side unit test exercises `TurnController`'s new `pendingCommands`/`drainPendingCommands` barrier directly (the new regression tests are server-only, proving persist-before-hydrate ordering via `handleCommand`, not the client-side drain call itself). A fake-hooks unit test asserting `onHumanTurnEnd` doesn't fire until a manually-resolved in-flight promise settles would close that gap.
7. ~~**Fight-screen UX fixes (G2, G7)**~~ — done in `src/screens/combat/arena/openManualBattleArena.ts` (2026-08-18): G2 hex under-cursor highlight on the canvas (distinct from `input.ts`'s directional-melee targeting); G7 `finishBattle()` now keeps the battlefield alive behind the result card (`closeArena()` runs from the card's Carry On button instead of firing first). Per-gap notes in `plan/2026-08-11-fight-screen-redesign.md`; tracked as "out of scope" in `plan/2026-08-17-combat-decomposition-finishing-breakout.md` §10 until this closure.
