...
| 1 | ✅ **DONE** — root npm workspaces (`packages/*`); `shared/` moved to `packages/engine` (the `shared/gameState.ts` re-export shim was retired in Stage 8/PR #78 once its last dependent, `applyEndOfTurnDetailed`, finished moving to `@heroes/engine`; `shared/` no longer exists); `packages/contracts` extracted (`ids.ts`, `geometry.ts`, `resources.ts`, `units.ts`, `buildings.ts`, `castle.ts`, `settlement.ts`, `gameState.ts`) | Physical boundary before anything else grows |
| 2 | ✅ **DONE** — carved `gameState.ts` into engine domains **one domain at a time** (economy → charter → settlement → hero → turn): `economy/*`+`charter/*` (PR #72, commit `f6ce118`), `settlement/*` (PR #74, commit `a0a638a`), `hero/*` (PR #75, commit `cd494b3`), `turn/*`+`calendar.ts` (PR #78). `gameState.ts` shrank from 894 to ~240 lines, but does **not** reach empty -- confirmed against `2026-08-10-deep-architecture-pass.md`'s explicit plan for it to permanently retain `isHuman`, init/seed (`createInitialState` + defaults), selection reducers (`selectHero`/`clearSelection`/`selectSettlement`/`clearSettlementSelection`), `markSaved`, and the type/re-export barrel. This row previously said the file "shrinks to re-exports until empty" -- that was aspirational, not literal; corrected here to avoid future confusion | Highest-traffic file; every future feature benefits immediately |
