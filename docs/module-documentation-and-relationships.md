...
| `server/routes.ts` | Core game API; orchestrates state mutations + combat | `../../shared/map/gameMap`, `../../shared/rng`, `../src/game/initState`, `../../shared/gameState`, `../../shared/units`, `../../shared/constants`, `../../shared/combat/resolveBattle`, `./assetRoutes`, `./auth` |
...
| `shared/combat/damage.ts` | Pure math: `typeMultiplier`, `computeDamage` (effAttack²/(effAttack+effDefense) × adv × mod), `applyCasualties`, `applyRetreatLoss` | `../units`, `../combatConfig` |
| `shared/combat/grid.ts` | `makeBattleGrid` (random obstacles via `mulberry32`), `deploymentPosition` | `../types`, `../rng`, `../combatConfig` |
...
| `shared/combat/manualBattle.ts` | HoMM3-style interactive engine: `startManualBattle`, `movePlatoon`/`attackWithPlatoon`, BFS movement range, line-of-sight, `runAiTurn`, `finalizeManualBattle` | `../types`, `../units`, `../combatConfig`, `./grid`, `./resolveBattle`, `./types` |
...
| `control.ts` | `controlRange`, `settlementRateRadius`, `controlledPositions`, `territoryBoundaryEdges` | `../../shared/types`, `../entities/settlement` |
| `buildingRegistry.ts` | Master `REGISTRY` of 13 building kinds (townHall, house, tower, mageGuild, mine, market, barracks, smithy, apartment, farmField, farmhouse, archeryRange, granary) — placement/upkeep/effects | `../../shared/types`, `../state/gameState` |
...
| `viewLauncher.ts` | View registration registry: views register themselves (show/hide/refresh hooks) here at module load so consumers (`ViewManager`, `UIManager`) reach them through a single entry point instead of direct cross-imports. Breaks view-to-view cycles. | `./menu`, `../state/gameState` (types only) |
| `homeView.ts` | Home overlay: New/Load/Settings/Sign-In modals, `userGames` remember; registers itself via `./viewLauncher` | `../io/api`, `../io/userGames`, `../io/auth`, `./menu`, `./settingsMenu`, `./viewLauncher` |
| `adventureView.ts` | **Main map view:** mouse drag/wheel/touch-pinch, hover tracking, path preview, click-to-move/adjacent-enemy/settlement, charter modal; `MAP_SEED=42`; registers itself via `./viewLauncher` | `../core/hex`, `../render/camera`, `../map/gameMap`, `../render/renderer`, `../entities/hero`, `../map/pathfinding`, `../state/gameState`, `../state/turnController`, `../render/overlays/pathOverlay`, `../managers/GameStateManager`, `./menu`, `../render/minimap`, `./viewLauncher` |
| `cityView.ts` | Full-screen city build view: keyboard (B/Esc/Delete/1–5 styles/!@#$%^ patterns/R reroll); mouse→grid picking; place/destroy/select modes; `TurnController.startBuildingUpgrade`; persists on close; wires `BuildingPlacer`, `BuildingMenu`, `BuildingSelectionMenu`, `CityDesignBoxManager`; registers itself via `./viewLauncher` | `../core/cityGrid`, `../render/cityRenderer`, `../map/resourceTiles`, `../render/assets`, `../render/cityBuildingDraw`, `../render/cityBuildingGen`, `./buildingMenu`, `./buildingPlacer`, `./buildingSelectionMenu`, `./confirmDialog`, `../state/settings`, `../state/gameState`, `../core/buildingRegistry`, `./CityDesignBoxManager`, `./viewLauncher` |
