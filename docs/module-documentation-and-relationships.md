...
| Module | Role | Imports |
|---|---|---|
| `menu.ts` | Generic popup primitives: `menuTheme`, `styleButton`/`styleInput`, `PopupMenu` class (draggable, closeable, setContent/appendContent/clearContent/setPosition, setOnClose for the caller's single close callback plus addCloseHandler for owned teardown that setOnClose cannot clobber), `openCenteredModal` (backdrop torn down via addCloseHandler) | — |
