...
| `debugCommands.ts` | Attaches `window.__gameDebug` for manual poking (`endTurn`, `requestMove`, `enterBattle`, etc.) | `../state/gameState`, `../map/pathfinding`, `../map/terrain`, `../core/hex` |
...
| `manualBattleArena.ts` | **Fullscreen HoMM3-style arena** for Test Battle: grid + side panels + footer (round/turn/time-of-day info row + action row with End Turn, **Retreat**, **Surrender** buttons); click-to-move/attack; alternates with `runAiTurn`; Retreat/Surrender call `retreatHero` (applyLoss true/false) → `finalizeManualBattle` → `showBattleResultCard`; Surrender costs SURRENDER_COST_GOLD (5000G) — if heroGold is insufficient, opens a Leave Behind picker (SURRENDER_UNIT_VALUE_GOLD=100 per unit) that strips the chosen counts from surviving platoons before finalize | `../core/hex`, `../../shared/combat/damage`, `../../shared/combat/manualBattle`, `../../shared/combat/types`, `../../shared/combatConfig`, `../state/units`, `./battleResultCard`, `./confirmDialog`, `./menu` |
...
| `developerSettingsMenu.ts` | Dev menu: event-bus inspector, Asset Manager launch, Test Battle launch | `./menu`, `../core/eventBus`, `./assetManager`, `./testBattleSetup` |
...
| `turnHooks.ts` | `buildTurnHooks` wires client reducers to API: `onHumanTurnEnd`→`/end-turn`, `onAiMove`→`/spend_movement`, `onBattleResolved`→`/resolve-battle`, `pickAiMove`→`aiBrain`, `logEvent`→`/events` | `../io/api`, `../state/gameState`, `../state/turnController`, `../ai/aiBrain`, `../map/gameMap`, `../core/hex` |
