...
| `cityRenderer.ts` | `drawCityView(ctx, nodes, deps, frame)` — four lines: `save` → `lineJoin = "miter"` → `paintScene` → `restore`. Decides nothing, draws nothing. The old `DrawCityViewOptions` bag is now `buildCityScene`'s input; the `?url` skybox imports moved to `./skybox.ts` | `../core/cityGrid`, `./scene/paint2d`, `./scene/types` |
| `overlays/pathOverlay.ts` | `computeReachableSplit` (called by the scene builder, not a painter) + `drawMinimapPath` (the minimap isn't in the scene graph). The path/trail drawing moved to `scene/paint2d`'s `paintPathSegment`/`paintHeroTrail` | `../../core/hex`, `../../map/gameMap`, `../../map/terrain`, `../minimapCamera`, `../renderTypes` |
| ~~`overlays/resourceIcon.ts`~~ | **Deleted 2026-08-21 (#148)** — replaced by the `resourceIcon` `SceneNode` + `paintResourceIcon` | – |
| ~~`overlays/territoryOutline.ts`~~ | **Deleted 2026-08-21 (#148)** — partitioning moved to `adventureScene.ts`'s `buildTerritoryOutlineEdges`, drawing to `paintTerritoryOutlineEdges` | – |
| `renderer.ts` | **Main per-frame world map orchestrator.** Exports `MapRenderer`. Background fill → `buildAdventureScene()` → `paintScene()` inside the camera `save`/`apply`/`restore` → `drawMinimap()` outside it. Holds one `Paint2DDep` for its lifetime. No raw drawing of its own beyond the background. Public surface (`new MapRenderer(ctx, map, camera, sprites, minimapCamera, mirror)`, `.draw(hover, heroes, path, castles, opts)`, `.hoverFromScreen(x, y)`, `.map` field, `.getMirror()`) — `mirror: EntityMirror` added as a required 6th constructor arg (#180 step 1); held on the instance but not yet consulted by `.draw()` | `../core/hex`, `./camera`, `../entities/hero`, `../entities/settlement`, `../map/gameMap`, `./assets`, `./fog`, `./minimap`, `./renderTypes`, `./minimapCamera`, `./scene/sceneBuilder/adventureScene`, `./scene/paint2d`, `./paint2dDefaults` |
| ~~`painter/*`~~ | **Deleted 2026-08-21 (#148).** Seven per-kind painter classes + barrel (`Background`, `HexTerrain`, `HexHover`, `SelectedTile`, `Hero`, `Castle`, `Charter`), added by PR #122. A parallel implementation of what `scene/paint2d/` already did; `paint2d/` survives. Divergence table in `src/render/docs/technical-spec.md` §7.3 | – |
| `scene/types.ts` | *(Phase 5 Track B — this **is** the live render path as of #148)* `SceneNode` discriminated union (`terrainHex`, `fogHex`, `resourceIcon`, `castle`, `hero`, `cityCell`, `cityBuilding`, `citySkybox`, …) + `WorldPoint`. The shared output type between the pure scene builders and the Canvas2D painter (and whatever replaces it) | `../map/terrain`, `../map/resourceTiles`, `../entities/hero`, `../state/settings`, `@heroes/contracts` |
| `scene/sceneBuilder/adventureScene.ts` | `buildAdventureScene()` — the sole source of *what* the adventure map draws, and in *what order*. Called by `MapRenderer.draw()` every frame. Takes `Hero[]`/`Castle[]`/`GameMap` wrappers rather than raw `GameState` (replacing that mirror is `entityMirror.ts`'s job). Optional `visible` input lets the renderer compute fog once and share it with the minimap | `../../../core/hex`, `../../../map/gameMap`, `../../../entities/hero`, `../../../entities/settlement`, `../../renderTypes`, `../../fog`, `../../overlays/pathOverlay`, `@heroes/engine`, `../types` |
| `scene/sceneBuilder/cityScene.ts` | `buildCityScene()` — same for the city interior; called by `CityView.draw()`. Owns what used to be `DrawCityViewOptions`. Skybox image loading/caching/parallax is a `paint2d`/`skybox.ts` concern, not a scene one; the `citySkybox` node only carries the resolved variant/parallax decision | `@heroes/engine`, `@heroes/contracts`, `../../../core/cityGrid`, `../../../map/resourceTiles`, `../../../state/settings`, `../../buildingStyles`, `../../cityBuildingDraw/primitives`, `../types` |
...
| `scene/entityMirror.ts` | `EntityMirror` — the visual `Hero[]`/`Castle[]` tween cache. `bootstrap(state)` hard-resyncs from `GameState`; `applyEvent(event)` handles `HeroMoved` (tween) and `SettlementCaptured` (owner) with every other `EngineEvent` a documented no-op for now; `update(dtMs)` ticks tweens. Meant to replace `GameEngine.ts`'s wholesale rebuild-on-`state:committed` pattern once Track 5.A's event-cursor stream exists. Reachable from `MapRenderer`/`ViewManager` as of #180 step 1 (via `getEntityMirror()` → constructor → `.getMirror()`), but still not consumed by `draw()`/`buildAdventureScene()` — that cutover is #180 step 2 | `../../entities/hero`, `../../entities/settlement`, `@heroes/contracts` |
| `scene/paint2d/` | `paintScene(ctx, nodes, deps, frame?)` — the Canvas2D consumer of `SceneNode[]`, and the **only** painter set in the repo since #148 deleted `render/painter/`. Switches on `node.kind` and dispatches to a real per-kind painter for all 27 kinds. Two kinds are run-batched to match the pass structure of the code they replaced: consecutive same-owner `territoryOutlineEdge` nodes stroke as one path (per-edge strokes double-blend shared endpoints at `globalAlpha` 0.45), and a run of `cityBuilding` nodes paints every body before any selection ring. The Vite-`?url` seam is enforced here: `paint2d/` declares a `Paint2DDep` interface (`deps.ts`) with four per-kind sprite resolvers (`resolveSpriteForResource/Hero/Building/Castle`) + `SkyboxProvider` + state getters + `colorForOwner`/`battleAccent`/`fontFamily`/`charterStyle`, so the painter never names a key string, never reads `settings()` directly, and never imports `assetDescriptors.ts`/`assets.ts`/`sprites.ts`/`cityRenderer.ts`/`cityBuildingDraw.ts` (the barrel). Charter palette constants in `colors.ts`; geometry helpers (`hexPath`, `diamondPath`) in `geometry.ts`; isometric building draw + offscreen style-leaf cache in `buildings.ts`. Boundary enforced by dependency-cruiser rules `paint2d-cannot-import-asset-descriptors` / `paint2d-cannot-value-import-state`, plus the runtime seam test `test/render/paint2d.seam.test.ts`. Consumers: `MapRenderer.draw()`, `drawCityView()`, and `arena/paint.ts`'s `paintSceneForArena()` (behind `?paint=scenebuilder`; see #143) | `../types`, `../../../core/hex`, `../../decorationSeed`, `../../../map/terrain`, `../../palettes`, `../../cityBuildingDraw/primitives` + the five per-style leaves — all leaf-clean, no Vite-`?url` coupling ever reaches the painter |
