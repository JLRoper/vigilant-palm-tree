...
| `manualBattleArena.ts` | **Fullscreen HoMM3-style arena** for Test Battle: grid + side panels + footer; click-to-move/attack; alternates with `runAiTurn`; ends with `finalizeManualBattle` → `showBattleResultCard` | `../core/hex`, `../../shared/combat/damage`, `../../shared/combat/manualBattle`, `../../shared/combat/types`, `../state/units`, `./battleResultCard`, `./menu` |
