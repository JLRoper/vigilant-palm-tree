...
A turn-based hex adventure map where the player moves a hero, claims resource tiles by building settlements on them, and defends them against enemy heroes that wander the map. Resources accumulate per turn and fund growth. Deeper systems (full army roster, tactical battlefield, fog of war, capture-for-ransom) are layered on later milestones without re-architecting the base.
...
| [settlements.md](./settlements.md) | Build cost, limits, capture, levels | ✅ Locked |
| [city-view.md](./city-view.md) | 10×10 settlement interior, mines, per-resource yield | 📋 Planned |
| [heroes.md](./heroes.md) | Hero movement, capture-for-ransom | ✅ Locked (movement) / ⏸️ Ransom deferred |
| [army.md](./army.md) | Unit roster, recruitment, food/upkeep, tactical combat | ⏸️ Deferred |
| [economy.md](./economy.md) | Per-turn economy flow tying resources + settlements | 📋 Planned |
| [map.md](./map.md) | Map generation, terrain, camera, fog of war (future) | 📋 Planned |
...
`map.md` defines the world. `resources.md` defines what's in it. `settlements.md` defines how the player claims them. `city-view.md` defines what happens inside them. `economy.md` defines the per-turn loop. `heroes.md` and `army.md` cover the actors.
...
- **Settlements:** build on resource tiles, max 3 per player, captured by enemy walking on them
- **Settlement cost:** flat **100g / 30w / 20s** regardless of resource rarity
- **Settlement destruction:** none in v1 — only capture changes ownership
- **Resources always visible** — no fog of war hiding them
- **Yield timing:** resources tick per player turn (end of turn)
- **Schema anticipates 3 levels** but only Level 1 ships in v1
- **City view:** double-click settlement → 10×10 grid → build mines on resource spots
